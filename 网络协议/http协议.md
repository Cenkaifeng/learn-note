...
- 协商缓存
当强制缓存失败后会再次向服务器发送请求，服务器需要比对客户端的缓存文件和服务店是否一致，如果一致则返回304状态。如果文件更新了，那么就采取返回新文件的方式。 头：Last-modified / if-modified-since
后续访问的时候发送请求和资源标识的字段名为If-Modified-Since放在请求头里面
