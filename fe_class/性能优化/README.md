...
- dom 阶段
  ...
  - js/css 等引用位置 
    js加载阻塞dom和cssmos执行， 因此js放下边，css放在上边
...
* Load: 传输内容到页面可交互的时间不超过5秒
  具体的方法：
    1. 优化阻塞资源标签
    2. chrome DevTool 有个分析必要js的功能(coverage)。参考那个把一些次要脚本剥离出来延迟加载。
  

## 工具

1. `web-vitals`: 一个前端框架，不需要你单独写 new performence api 
...
5. [Measure](https://web.dev/measure/) 和 webpagetest 类似的在线测试网站
6. [Jspref](https://github.com/jsperf/jsperf.com) 一个用于测试 js 代码性能的工具（相关在线网站需要Github进行登录），可以本地按照脚本来使用.
