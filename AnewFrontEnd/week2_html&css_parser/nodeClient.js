    ...
    if (this.current === this.WAITING_STATUS_LINE) {
      ...
      if (char === ":") {
        ...
        if (this.headers["Transfer-Encoding"] === "chunked")
          // Transfer-Encoding有很多值 node 默认 chunced
...
void (async function () {
  ...
  console.log("nodeClient.js response:???????????????", response.body);
  let dom = parser.parseHTML(response.body); // 如果这里写的是真正的浏览器，会将 response.body 逐段的返回parser
  // console.log("nodeClient.js", dom);
})();
